...
def generate_line(n: int) -> None:
    ...
    print(
        f"{np.linalg.norm(ans_normal - np.ones(n), 2):^16.6f}|"
        f"{np.linalg.norm(ans_normal - np.ones(n), np.inf):^18.6f}|"
        f"{np.linalg.norm(ans_all_max - np.ones(n), 2):^17.6f}|"
        f"{np.linalg.norm(ans_all_max - np.ones(n), np.inf):^19.6f}|"
        f"{np.linalg.norm(ans_column_max - np.ones(n), 2):^20.6f}|"
        f"{np.linalg.norm(ans_column_max - np.ones(n), np.inf):^21.6f}|"
    )
